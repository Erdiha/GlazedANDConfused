...
function SharedContainer({ images }: IImages) {
  ...
  return (
    <div className='flex flex-col w-full h-full justify-around   items-center  '>
      <div className='flex h-full w-[75%] max-w-[100rem] bg-[#FAF3F0] p-20 justify-center items-center flex-col gap-16  mt-20'>
        {images.map((image, index) => (
          <motion.div
            ...
            whileHover={{
              ...
              borderLeft: `${index % 2 === 0 ? '#ff52a2 solid 8px' : 'none'}`,
              borderRight: `${index % 2 === 1 ? '#ff52a2 solid 8px' : 'none'}`,
              transition: { duration: 0.5, delay: 0, ease: 'easeInOut' },
            }}
            key={index}
            className='flex justify-center flex-col md:flex-row  items-center rounded-[15px] shadow-xl'>
            ...
            {image.src !== 'na' && (
              <div className=' justify-center flex flex-col p-2 h-full w-full '>
                <p
                  ...
                  className=' flex  text-black font-bold p-2 capitalize tracking-wide leading-[2.7rem]'>
                  {image.title}
                </p>
                <p
                  style={{ fontSize: mbl ? '0.8rem' : '1.2rem' }}
                  className={`text-black/90 p-2 font-serif leading-loose tracking-wide `}>
                  {image.description}
                </p>
              </div>
            )}
